...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_dir', type=str, help="input image dir path")
    ...
    if os.path.isdir(opts.input_dir):
        input_images = [os.path.join(opts.input_dir, f) for f in os.listdir(opts.input) if f.lower().endswith(('png', 'jpg', 'jpeg'))]
    else:
        input_images = [opts.input_dir]
    ...
    with torch.no_grad():
        ...
        for input_image in input_images:
            image = Variable(transform(Image.open(input_image).convert('RGB')).unsqueeze(0).cuda())
