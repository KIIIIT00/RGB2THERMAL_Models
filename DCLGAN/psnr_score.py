...
# 結果のフォルダ
RESULT_FOLDER = f"./DCLGAN/results/{model_name}/test_latest/images/"
# ファイル名の一覧を取得
files = os.listdir(RESULT_FOLDER)
real_folder_name = 'real_B'
fake_folder_name = 'fake_B'
real_folder_path = os.path.join(RESULT_FOLDER, real_folder_name)
fake_folder_path = os.path.join(RESULT_FOLDER, fake_folder_name)

real_list = os.listdir(real_folder_path)
fake_list = os.listdir(fake_folder_path)
# real = 'real.jpg'
# fake = 'fake.jpg'
...
for real, fake in zip(real_list, fake_list):
    ...
    print("Fake:", fake)
    print("--------------------------------")
    real_img = cv2.imread(os.path.join(real_folder_path, real))
    fake_img = cv2.imread(os.path.join(fake_folder_path, fake))
...
print(f"MSE: {mse / len(real_list):.{precision}f}")
print(f"RMSE: {rmse / len(real_list):.{precision}f}")
print(f"PSNR: {psnr / len(real_list):.{precision}f}")
print(f"SSIM: {ssim / len(real_list):.{precision}f}")
print(f"LPIPS(Alex): {lpips_score_alex / len(real_list):.{precision}f}")
print(f"LPIPS(VGG): {lpips_score_vgg / len(real_list):.{precision}f}")
print(f"LPIPS(Squeeze): {lpips_score_squeeze / len(real_list):.{precision}f}")
