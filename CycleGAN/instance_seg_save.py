...
def process_images_in_folder(folder_paths, model, mask_color_dict, output_dir):
    ...
    for original_path in original_images:
        ...
        # 保存ファイル名
        seg_original_path = os.path.join(os.path.join(output_dir, f"image_{idx}"), f"original_seg_{idx}.jpg")
        seg_thermal_path = os.path.join(os.path.join(output_dir, f"image_{idx}"), f"thermal_seg_{idx}.jpg")
        seg_ge_thermal_path = os.path.join(os.path.join(output_dir, f"image_{idx}"), f"gen_thermal_seg_{idx}.jpg")
        
        overlay_image_RT_path = os.path.join(os.path.join(output_dir, f"image_{idx}"), f"overlay_{idx}_RT.jpg")
        overlay_image_TG_path = os.path.join(os.path.join(output_dir, f"image_{idx}"), f"overlay_{idx}_TG.jpg")
        
        overlay_legend_RT_path = os.path.join(os.path.join(output_dir, f"image_{idx}"), f"overlay_with_legend_{idx}_RT")
        overlay_legend_TG_path = os.path.join(os.path.join(output_dir, f"image_{idx}"), f"overlay_with_legend_{idx}_TG")
...
OUTPUT_DIR = f'./CycleGAN/instance_seg/{pretrained_model_name}/'
